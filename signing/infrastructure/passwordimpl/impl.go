...
func (impl *passwordImpl) hasMultiChars(s []byte) bool {
	...
	for _, c := range s {
		part[impl.getCharTypeIndex(c)] = true
	}

	return countTrue(part) >= impl.cfg.MinNumOfKindOfPasswordChar
}

func (impl *passwordImpl) getCharTypeIndex(c byte) int {
	switch {
