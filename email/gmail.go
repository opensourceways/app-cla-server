...
var gmailCfg *oauth2.Config
var gmailEmailTemp *template.Template
...
func GetGmailToken(code, scope string) (*oauth2.Token, error) {
	if gmailCfg == nil {
		return nil, fmt.Errorf("gmail has not been initialized")
	}

	return myoauth2.FetchOauth2Token(gmailCfg, code)
}

func GetGmailAuthorizedEmail(token *oauth2.Token) (string, error) {
	ctx := context.Background()
	srv, err := gooleoauth2.NewService(
		ctx, option.WithTokenSource(gmailCfg.TokenSource(ctx, token)))
	if err != nil {
		return "", err
	}
...
func GetGmailOauth2CodeURL(state string) string {
	return myoauth2.GetOauth2CodeURL(gmailCfg, state)
}

func (this *gmailClient) SendEmail(msg *EmailMessage) error {
	client := gmailCfg.Client(context.Background(), msg.SendInfo.Token)
...
func (this *gmailClient) createGmailMessage(msg *EmailMessage) (*gmail.Message, error) {
	...
	err = gmailEmailTemp.Execute(buf, data)
	if err != nil {
