...
func (this *txmailClient) SendEmail(token *oauth2.Token, authorizeCode string, msg *EmailMessage) error {
	m, err := this.createTxMailMessage(msg)
	if err != nil {
		return err
	}

	d := gomail.NewDialer(this.cfg.host, this.cfg.port, msg.From, authorizeCode)
